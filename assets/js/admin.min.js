...
/**
 * Adds an Accordion effect to categories (fieldsets)
 * in configuration or language input forms.
 *
 * @since 1.8
 */
var acc = document.getElementsByClassName("xhAccordionTitle");
var i;
...
function openAll() {
    ...
    var element = document.getElementById("xhAccAllOpen");
    element.classList.add("xhAccAllDeactivated");
    var element = document.getElementById("xhAccAllClose");
    element.classList.remove("xhAccAllDeactivated");
}
...
var el = document.querySelector("#xh_config_form .xhAccAllOpen");
if (el) {
    el.addEventListener('click', openAll, false);
}
var el = document.querySelector("#xh_config_form .xhAccAllClose");
if (el) {
    el.addEventListener('click', closeAll, false);
}

/* Plugins - if only 5 accordians, show this expanded
   begins at 0*/
var xhAcc = document.getElementById("xh_config_form");
for (i = 0; i < acc.length; ++i) {
    if (i <= 4) {
        xhAcc.classList.add("xhAccOpenLimited");
    } else {
        xhAcc.classList.remove("xhAccOpenLimited");
    }
}
